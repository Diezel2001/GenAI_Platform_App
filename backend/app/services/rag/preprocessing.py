...
import app.services.rag.utilities as utilities
import app.services.rag.rag_schemas as rag_schemas
...
def _extract_local_raw_document(path: str, ftype: str) -> rag_schemas.RawDocument:
    stat = os.stat(path)
    mime_type, _ = mimetypes.guess_type(path)

    return rag_schemas.RawDocument(
        source_type="local",
        file_type=ftype,
        path_or_url=path,
...
def _extract_uploaded_raw_document(upload_file, ftype: str) -> rag_schemas.RawDocument:
    """Extract raw document from an uploaded file."""
...
def _extract_remote_raw_document(path: str, ftype: str) -> rag_schemas.RawDocument:
...
def extract_raw_document(path: str) -> rag_schemas.RawDocument:
